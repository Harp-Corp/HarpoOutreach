...
extension AppViewModel {
    ...
    func draftEmail(for leadID: UUID) async {
        ...
        guard !leads[idx].email.isEmpty else {
            errorMessage = "Keine Email-Adresse für \(leads[idx].name)."; return
        }
    ...
    func draftAllEmails() async {
        for lead in leads.filter({ !$0.email.isEmpty && $0.draftedEmail == nil }) {
